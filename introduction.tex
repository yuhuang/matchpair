...
As for a verification process, the approach in this paper starts with $k=1$ and increments $k$ to a larger bound that is able to change the number of the divided sections, as the new set of match pairs is only meaningful if each process is sectioned differently. The growth of the match pair set is polynomial as $k$ increases. The program property is then witnessed with the larger bound $k$; or the process repeats.
