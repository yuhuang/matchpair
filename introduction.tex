...
First, each process is sectioned, where a section contains a sequence of receives in the process and a bounded number ($k$) of sends from each sender that may potentially match these receives. The bound $k$ is input by the user. 
...
As for a verification process, the approach in this paper starts with $k=1$ and increments $k$ to a larger bound that is able to change the number of the divided sections, because the new set of match pairs is only meaningful if each process is sectioned differently. The program property is then witnessed with the larger bound $k$; or the process repeats.
...
The paper includes the proof that the precise match pairs for any CTP can be generated with an appropriate bound $k$. 
